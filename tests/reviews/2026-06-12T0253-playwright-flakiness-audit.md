...
| ~~P3~~   | ~~Add `indexedDB: true` to storageState calls~~ ⛔ deferred — BCM LWC has no IndexedDB usage (grep-confirmed); SF session auth is cookie-based. Revisit if a dependency adds client-side IndexedDB state.                                                     | `tests/e2e/fixtures/auth.setup.ts`                                                                                                                                                                                                   |
| ~~P3~~   | ~~Add `test.slow()` to known-slow describe blocks~~ ⛔ deferred — no timeout failures observed in practice; 60s global budget sufficient. Revisit if CI shows consistent timeout flakes on drag-drop or import.                                               | `drag-drop.spec.ts`, `capability-detail.spec.ts`                                                                                                                                                                                     |
| ~~P3~~   | ~~Replace `waitForTimeout` in `__visual__/capture.spec.ts`~~ ✅ done 2026-06-12 — focus tests use `data-focused="true"`; toggles wait for `.bcm-strategy-stripe`/`.bcm-band-node`; tag waits for option to disappear; detail panel polls CSS transform matrix | `tests/e2e/__visual__/capture.spec.ts`                                                                                                                                                                                               |
| ~~P4~~   | ~~Document MFA status; add TOTP handling if MFA ever enabled~~ 🚫 will not do                                                                                                                                                                                 | `tests/e2e/fixtures/auth.setup.ts`                                                                                                                                                                                                   |
| ~~P4~~   | ~~Add quarantine tag convention + pre-merge stress-run gate to CONTRIBUTING~~ 🚫 will not do                                                                                                                                                                  | new `CONTRIBUTING.md`                                                                                                                                                                                                                |
