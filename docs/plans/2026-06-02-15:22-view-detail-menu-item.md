...
`bcm_ContextMenu` currently gates the "View detail" item behind `<template if:true={isL3}>`, so L1/L2 nodes get a menu with only "Hide". Spec (`docs/specs/diagram.md` Feature: Context menu actions — *View detail opens the Detail Panel*) requires the item on **all three levels**.
...
**Spec changes:**
- `docs/specs/diagram.md` — adds one new scenario inside *Feature: Context menu actions* (item visibility) Tested-by Jest. Umbrella scenario already exists and stays pointed at the panel slice.
