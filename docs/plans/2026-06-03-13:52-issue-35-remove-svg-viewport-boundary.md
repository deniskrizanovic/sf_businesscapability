...
**Goal:** Capability map diagram remains fully visible regardless of pan or zoom. Drop the implicit `<svg>` clipping rectangle (it's bound to `canvasWidth`/`canvasHeight` which are sized for the _initial_ layout, not for translated/scaled content). Also remove the asymmetric `panY <= 0` clamp so users can pan in all four directions.
...
- **Why not change `canvasWidth`/`canvasHeight`:** keeping them stable means `Fit to Window` math (`cw / dw`, `ch / dh`) and the container scrollbars all keep working; only the _clip_ changes.
