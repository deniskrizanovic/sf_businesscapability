...
| Render order               | `<rect>` before `<text>` so text sits on top — emit rect at the _first_ bullet line of the focused L3 only; text continues to be rendered per line     |
...
- On the first line only, emit a `focusRect` object when `l3Focused`:
    ...
    ```
    (`bulletY` value captured _before_ the inner `forEach` increments it.)
...
<template for:each="{node.bulletLines}" for:item="bullet">
    ...
    <text ... font-weight="{bullet.fontWeight}"> {bullet.text} </text>
</template>
```
