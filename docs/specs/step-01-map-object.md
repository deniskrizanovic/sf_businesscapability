...
**Scenario: Map object is deployed**
- Given the metadata has been deployed to the org
- When a user navigates to Setup → Object Manager
- Then `bcm_Map__c` is listed as a custom object

**Scenario: Description field is present and is Rich Text**
- Given the `bcm_Map__c` object exists
- When a user views the Fields & Relationships for `bcm_Map__c`
- Then `bcm_Description__c` is present with type Rich Text Area
